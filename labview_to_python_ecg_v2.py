...
# -*- coding: utf-8 -*-
"""
ECG Seizure Pipeline (LabVIEW-parallel)
=======================================
- R-peak detection:
  * mode="simple"  : Pan–Tompkins-like (bandpass→derivative→square→MVI→adaptive threshold)
  * mode="labview" : Jeppesen LabVIEW algorithm (2 s windows, Thigh/Tlow, Rmax, DELTA, fwd/bwd peak)
- RR → HRV features:
  * 7-beat median filter on RR
  * Lorenz SD1/SD2 → T, L → CSI, ModCSI
  * Tachogram slope (100-beat LSQ on HR vs. time)
  * Combined features: ModCSI_filt×slope, CSI_raw×slope
- Detection by thresholds (manual or auto-baseline).

Usage:
  python labview_to_python_ecg_v2.py --in ecg.csv --fs 256 --time_col t --ecg_col ecg \
    --rpeak_mode labview --out run_lab --auto_baseline_beats 20000 --plots
"""
...
class LabVIEWRpeak:
    ...
    def _window_max_series(self, sig):
        """Max per 2s window on rectified filtered ECG (negatives→0)."""
    ...
    def detect(self, ecg):
        ...
        # Tlow as scaled Thigh (per VI-noter)
        tlow = 0.4*thigh
        ...
        while i < len(bp):
            ...
            # Shadow detection marker ved vinduesgrænse (holder Tlow-adfærd som i VI'en)
...
def main():
    ap = argparse.ArgumentParser(description="ECG seizure pipeline (LabVIEW-parallel)")
    ...
    ap.add_argument("--rpeak_mode", type=str, choices=["simple","labview"], default="simple")
    ...
    if not args.rr_csv and not args.in_csv:
        print("Provide either --rr_csv or --in <ECG.csv>")
        sys.exit(1)

    if args.rr_csv:
        rr_sec, t_rr, _ = load_ecg_or_rr(args)
        fs_used = None
    else:
        ecg, t, fs = load_ecg_or_rr(args)
        fs_used = fs
