...
# -*- coding: utf-8 -*-
"""
BUT QDB — Ultra-light Noise Baseline (v2)
Focus: compare simple noise measures to consensus classes 1/2/3.
Folder layout (PhysioNet):
  ROOT/<rid>/<rid>_ECG.dat
  ROOT/<rid>/<rid>_ECG.hea
  ROOT/<rid>/<rid>_ACC.*   (unused here)
  ROOT/<rid>/<rid>_ANN.csv  <-- NOTE: annotation file name
"""
...
def read_butqdb_annotations(csv_path: Path, n_samples: int):
    """
    BUT QDB: 12 columns; last 3 columns are consensus start/stop/class (0-based: 9,10,11).
    The official annotation filename is <rid>_ANN.csv (PhysioNet page).
    """
...
def find_annotations(subdir: Path, rid: str):
    # Official filename: <rid>_ANN.csv
    for name in [f"{rid}_ANN.csv", f"{rid}_ann.csv", f"{rid}_Ann.csv"]:
        p = subdir / name
        if p.exists():
            return p
    # Fallbacks: anything with 'ann' (not only 'annot') in name
    hits = [q for q in subdir.glob("*.csv") if "ann" in q.name.lower()]
    return hits[0] if hits else None
...
def main():
    ap = argparse.ArgumentParser(description="BUT QDB — Ultra-light Noise Baseline (v2)")
