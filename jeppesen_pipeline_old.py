...
class LVParams:
    ...
    # Sample-baserede konstanter (angivet som 256 Hz defaults)
    fwd_bwd_radius_samp: int = 15       # ±15 @256 Hz  (skal skaleres i koden til fs)
    delta_thresh_samples: int = 35      # 35 @256 Hz    (skal skaleres i koden til fs)
    ...
    # NYT: slå Tlow-searchback til/fra
...
class LabVIEWRpeak:
    ...
    def detect(self, ecg):
        ...
        use_tlow = use_tlow and self.p.enable_tlow
        if use_tlow:
            tlow = 0.4 * thigh
            ...
            if not use_tlow:
                ...
                if len(rr_list) >= 3:
                    ...
                    if seg_rr.size >= 5:
                        ...
                        high_var = (delta_val * fs > 1.4 * self._delta_thr)  # i stedet for bare > self._delta_thr

                    else:
                        high_var = False
                    lookback_rr = self.p.rrshort_n if high_var else self.p.rrlong_n
                    lookback_s = min(lookback_s, 1.1 * rmax)  # søg højst til ~110% af Rmax
                else:
                    lookback_s = 2.0
                ...
                # efter du har fundet 'ci' (kandidatpeak)
                if ci>0 and ci<len(bp)-1:
                    if not (bp[ci] >= bp[ci-1] and bp[ci] >= bp[ci+1]):
                        # ikke tydeligt lokalt max -> drop
                        ci = None
...
def _preprocess_labview_bandpass(
    ecg, fs,
    delete_start_s=60.0, delete_end_s=300.0,
    hp_stop=1.0, hp_pass=1.2,
    lp_pass=32.0, lp_stop=50.0,
    smooth_win_samples=4
):
    ...
    if n0 + n1 >= len(x):
        raise ValueError("delete_start_s + delete_end_s fjerner hele signalet.")
    x = x[n0: len(x) - (n1 if n1 > 0 else 0)]

    nyq = fs/2.0
    if not (0 < hp_stop < hp_pass < nyq):
        raise ValueError("HP kræver: 0 < hp_stop < hp_pass < fs/2.")
    if not (0 < lp_pass < lp_stop < nyq):
        raise ValueError("LP kræver: 0 < lp_pass < lp_stop < fs/2.")

    # Equi-ripple High-pass (Parks–McClellan)
    taps_hp = remez(
        numtaps=101,
    ...
    x = filtfilt(taps_hp, [1.0], x, padlen=min(300, len(x)-1))

    # Equi-ripple Low-pass
    taps_lp = remez(
        numtaps=101,
    ...
    x = filtfilt(taps_lp, [1.0], x, padlen=min(300, len(x)-1))

    # 4-sample moving average (rectangular)
    if smooth_win_samples > 1:
        ker = np.ones(int(smooth_win_samples))/float(smooth_win_samples)
        x = np.convolve(x, ker, mode="same")

    return x, n0  # returnér også hvor mange samples vi klippede i starten


# ----------------- Fallback detektor med auto-polaritet -----------------
def _detect_labview_with_autopol_fallback(ecg, fs, params=None, auto_polarity="global16s"):
    """
    Bruges kun hvis LabVIEWRpeak/LVParams ikke har auto_polarity i dit modul.
    """
    import numpy as _np
    x = _np.asarray(ecg, float).copy()

    if auto_polarity == "global16s":
        n = int(16*fs)
        seg = x[:min(len(x), n)]
        if seg.size:
            pos95 = _np.percentile(seg, 95)
            neg05 = _np.percentile(seg, 5)
            if abs(neg05) > pos95:
                x *= -1.0
        ...
        for st in range(0, len(x), L):
            en = min(len(x), st+L)
            seg = x[st:en]
            if seg.size and abs(_np.min(seg)) > _np.max(seg):
                x[st:en] = -seg

    # fs-skalering af sample-baserede konstanter (±15 og DELTA=35 @256 Hz)
    scale = fs/256.0
    p = params or LVParams(fs=fs)
    p = LVParams(
        fs=fs,
        win_s=getattr(p, "win_s", 2.0),
        thigh_alpha=getattr(p, "thigh_alpha", 0.75),
        fwd_bwd_radius_samp=int(round(getattr(p, "fwd_bwd_radius_samp", 15)*scale)),
        delta_thresh_samples=int(round(getattr(p, "delta_thresh_samples", 35)*scale)),
        rrshort_n=getattr(p, "rrshort_n", 8),
        rrlong_n=getattr(p, "rrlong_n", 34),
        refractory_s=getattr(p, "refractory_s", 0.25),
        rmax_clip_low_s=getattr(p, "rmax_clip_low_s", 0.4),
        rmax_clip_high_s=getattr(p, "rmax_clip_high_s", 1.2),
    )
    peaks = LabVIEWRpeak(p).detect(x)
    return peaks, x


# ===================== Main: run_pipeline_from_tdms_auto =====================
...
def run_pipeline_from_tdms_auto(
    ...
    *,
    ...
    ui_defaults: bool = True,          # sæt VI-UI standarder
    # preproc overrides:
    ...
    auto_baseline_beats: int = 20000,
    auto_pct: float = 99.5,
    plots: bool = True,
    out: str = "run"
):
    """
    TDMS -> (valgfri slice) -> LabVIEW-lignende preproc -> LabVIEW R-peak -> HRV-features -> detections.
    Returnerer dict med peaks, rr_sec, t_rr, features, detections, meta.
    """
    # --- TDMS load ---
    x, t_sec, fs, *meta_rest = read_tdms_auto(tdms_path, fs_override=fs_override)
    ...
    if preproc == "labview":
        ...
        # opdater tidsakse hvis vi har en
        if t_sec is not None:
            t_sec = t_sec[n0:n0+len(x_proc)]
    else:
        x_proc = x  # ingen preproc

    # --- R-peak detection (LabVIEW-mode 1:1) ---
    if rpeak_mode == "simple":
        peaks = rpeaks_simple(x_proc, fs)
        x_pol = x_proc
    else:
        # Forsøg at bruge ny LVParams(auto_polarity) hvis den findes
        use_new = hasattr(LVParams, "__annotations__") and "auto_polarity" in LVParams.__annotations__
        if use_new:
            lv = LabVIEWRpeak(LVParams(
                fs: float = 256.0
                win_s: float = 2.0
                thigh_alpha: float = 0.75
            ...
            x_pol = x_proc  # signalet er polaritetsrettet inde i detektoren
        else:
            # Fallback der laver auto-pol + fs-skalering uden at ændre modulklassen
            peaks, x_pol = _detect_labview_with_autopol_fallback(
                x_proc, fs, auto_polarity="global16s"
            )

    # --- RR, features, detection ---
    rr_sec, t_rr = rr_from_peaks(peaks, fs)
    features, detections, meta = _finish(
        rr_sec, t_rr, out, plots,
        thr_modcsi_x_slope=None,   # behold dine defaults i _finish
        thr_csi_x_slope=None,
        auto_baseline_beats=auto_baseline_beats,
        auto_pct=auto_pct,
        window_beats=window_beats
    )
    ...
    meta_out = {
        ...
        "delete_start_s": delete_start_s,
        "delete_end_s": delete_end_s,
        "hp_stop": hp_stop, "hp_pass": hp_pass,
        "lp_pass": lp_pass, "lp_stop": lp_stop,
        "smooth_win_samples": smooth_win_samples,
    ...
    return {
        ...
        "meta": meta_out,
        "ecg_proc": x_pol,         # det polaritets-korrigerede, pre-processede signal
        "t_proc": t_sec            # (kan være None)
    }
