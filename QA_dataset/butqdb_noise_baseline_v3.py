...
# -*- coding: utf-8 -*-
"""
BUT QDB — Ultra-light Noise Baseline (v3)
- Robust summary() that never KeyErrors even if only class 0 appears.
- Skips windows where y3==0 (tie/unknown), focusing strictly on classes 1/2/3.
- Discovers records as ROOT/<rid>/<rid>_ECG.* + <rid>_ANN.csv (or any *ann*.csv)
"""
...
# -----------------------------
# Discovery (one-level subfolders)
# -----------------------------
...
def find_annotations(subdir: Path, rid: str):
    for name in [f"{rid}_ANN.csv", f"{rid}_ann.csv", f"{rid}_Ann.csv"]:
        p = subdir / name
        if p.exists():
            return p
    hits = [q for q in subdir.glob("*.csv") if "ann" in q.name.lower()]  # lenient
    return hits[0] if hits else None
...
def build_features_for_item(item, win_sec=8.0, step_sec=2.0):
    ...
    for s,e in window_indices(len(ecg), fs, win_sec, step_sec):
        if np.all(mask[s:e] == 0):
            continue
        y3 = label_window(mask, s, e)
        if y3 == 0:
            continue  # skip ambiguous/ties
...
def summarize(df: pd.DataFrame):
    features = ["rms","bw_ratio","hf_ratio","pl_ratio","zcr"]
    if df.empty:
        return pd.DataFrame(columns=["class"] + features + ["n"])

    # robust groupby
    agg = df.groupby("y3")[features].median()
    agg.index.name = "class"
    agg = agg.reset_index()

    cnt = df["y3"].value_counts(dropna=False)
    cnt.index.name = "class"
    cnt = cnt.reset_index(name="n")

    out = pd.merge(agg, cnt, on="class", how="outer")
    # keep only 1..3, reindex to ensure rows exist for all
    out = out[out["class"].isin([1,2,3])]
    out = out.set_index("class").reindex([1,2,3])
    for f in features:
...
def main():
    ap = argparse.ArgumentParser(description="BUT QDB — Ultra-light Noise Baseline (v3)")
    ...
    ap.add_argument("--dryrun", action="store_true", help="Only list discovered records and exit")
    args = ap.parse_args()
    ...
    if not items:
        raise SystemExit("No records discovered under --root. Expecting ROOT/<rid>/<rid>_ECG.* + <rid>_ANN.csv")
